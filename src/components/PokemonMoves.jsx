...
function PokemonMoves({ filter, moves }) {
  ...
  useEffect(() => {
    const filteredMoves = moves.filter((move) => {
      return move.version_group_details[0].move_learn_method.name === filter;
    });

    Promise.all(
      filteredMoves.map((move) => {
        return POKEDEX.getMoveByName(move.move.name).then((fetched) => ({
          ...move,
          ...fetched,
        }));
      })
  ...
  return (
    <div className={classes.root}>
      <Typography>
        Pokemon Moves ({filter}) ({displayedMoves.length})
      </Typography>
